...
const EventForm: FunctionComponent<TEventForm> = ({ eventId, eventType, isEditMode, eventData}) => {
  ...
  const { control, watch, handleSubmit, setValue, unregister, clearErrors, formState: { errors } } = useForm<IEventForm>(); 
    ...
    <form className={classes.event_form} onSubmit={handleSubmit(onEventFormSubmit)}>
      <div className='left_side_form'>
              ...
              render={({ field }) => {
                return (
                  <TextField
                    ...
                    label="Title"
                    fullWidth
                    className={`${!!errors.title ? classes.error_field : ''}`}
                    variant="outlined"
                    inputProps={
                      {
                        readOnly: eventType === 'new' ? false : !isEditMode
                      }
                    }
                  />
                )
              ...
              render={({ field }) => {
                return (
                  <TextareaAutosize
                    ...
                    disabled={eventType === 'new' ? false : !isEditMode}
                  />
                )
              }}
            />
        ...
        <div className={classes.form_block_wrap}>
          ...
          {
            (watchSelectDateStart && watchSelectDeadline && (new Date(watchSelectDeadline) > new Date(watchSelectDateStart)) || eventData?.startDate && eventData.deadline) &&
            <div className={classes.input_wrap}>
              <Controller
            ...
            >
              ...
              <input
                ...
                onChange={(event: ChangeEvent<HTMLInputElement>) => {
                  ...
                  (imageFile && imageFile.name.match(/\.(jpg|jpeg|png|gif|ico)$/)) ? setValue('image.imageData', imageFile) : alert('Ошибка, нужна картинка');
                }}
                hidden
              />
            </Button>
